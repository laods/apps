...
int main(int varnum, char** vararg) {
    ...
    if (section_name == "SN74" || section_name == "sn74") {
        auto init = initializer_list<double>({23771,86713,1987,50694,28470,66404,26280,58528,4547,79796,61050,10623});
	copy(init.begin(), init.end(), Pc_avg);
	init =initializer_list<double>({3038,12617,679,3712,5526,8420,4949,9626,1440,14447,8060,3386});
	copy(init.begin(), init.end(), Pb);
	...
	map_poro_satnum[0.24] = 12;
        ...
        auto init = initializer_list<double>({23771,86713,50694,20412,2882,31040,4547,79796,20369,10623});
	copy(init.begin(), init.end(), Pc_avg);
	init = initializer_list<double>({3038,12617,3712,1630,1153,10224,1440,14447,1573,3386});
	copy(init.begin(), init.end(), Pb);
	map_poro_satnum[0.1933] = 1;
	map_poro_satnum[0.1033] = 2;
	...
	map_poro_satnum[0.24] = 10;
        ...
        auto init = initializer_list<double>({23771,86713,1987,50694,78336,3516,9222,66404,6282,5675,20412,26280,4547,79796,7158});
	copy(init.begin(), init.end(), Pc_avg);
	init = initializer_list<double>({3038,12617,679,3712,17123,1327,1869,8420,1188,1401,1630,4949,1440,14447,1775});
	copy(init.begin(), init.end(), Pb);
	map_poro_satnum[0.1933] = 1;
	map_poro_satnum[0.1033] = 2;
	map_poro_satnum[0.27] = 3;
	...
	map_poro_satnum[0.2605] = 15;
        ...
        auto init = initializer_list<double>({1.2286e4,2.7473e5});
	copy(init.begin(), init.end(), Pc_avg);
	init = initializer_list<double>({4554.9,1.0185e5});
	copy(init.begin(), init.end(), Pb);
